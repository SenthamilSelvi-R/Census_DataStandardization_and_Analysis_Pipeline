...
# Function to create a streamlit window with visuals:
def window(window_title, query, chart_type=None):
    ...
    # Create visualization based on the chart_type parameter
    if chart_type == 'bar_chart':
        st.bar_chart(out_df.set_index(columns[0]))  # Assumes the first column is categorical, e.g., 'State/UT'
    
    elif chart_type == 'pie_chart':
        # Plot pie chart using matplotlib and show in Streamlit
        plt.figure(figsize=(8, 8))
        plt.pie(out_df[columns[1]], labels=out_df[columns[0]], autopct='%1.1f%%', startangle=140)
        plt.title(f"{columns[1]} Distribution by {columns[0]}")
        st.pyplot(plt)

    elif chart_type == 'line_chart':
        st.line_chart(out_df.set_index(columns[0]))
...
def window_1():
    window('Total population of each District',"SELECT District, Population FROM Census_DB.census_2011 GROUP BY District LIMIT 5 ",chart_type='bar_chart')

# Window 2 : Literate male and female in each district

def window_2():
    window('Literate Male and Female in each district', \
            "SELECT District_Code, State_UT, District, Literate_Male, Literate_Female FROM Census_DB.census_2011 GROUP BY Distri")
...
def window_11():
    window('Total number of households in each state',
            ...
            FROM Census_DB.Census_2011 \
            GROUP BY State_UT', chart_type='pie_chart')
